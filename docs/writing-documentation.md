# Writing Documentation — omp-config

Materialized from `'/home/ashby/.omp/agent/skills/write-documentation'` (the canonical source in this repo) — refresh this copy when that skill changes. The review bot enforces only what it reads (`repo_context_files`), so the full standard lives here as well as in the skill.
