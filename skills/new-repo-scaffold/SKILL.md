...
- **Pipeline logic is make-only**: never inline `pip install`/`npm test` logic into the workflow — anything deciding what ships or what the run concludes (build, test, lint, the coverage gate) lives behind a make target; tool bootstrap and result plumbing (checkout, `setup-*`, `upload-artifact`, the coverage actions) may be workflow steps. Every action pins to a commit SHA with its version as a trailing comment — never the mutable tag.
