...
- **The reviewer's enforcement is scoped to `repo_context_files`.** It can only enforce rules present in the files it reads — so the scaffold materialises the **canonical global standard** (`docs/coding-standards.md` in the omp-config repo, where this skill lives) **in full** into each repo's `docs/coding-standards.md`, then project rules are added on top. Once materialised, every rule is enforced (one repo shipped a condensed copy and the bot never commented on style until the full rules were present).
...
- `docs/` standards triplet, checked by PR-Agent:
  - `docs/coding-standards.md` — materialised **in full** from the **canonical global standard** (`docs/coding-standards.md` in the omp-config repo, where this skill lives), then project rules added; the reviewer enforces exactly the rules present in the file, so materialise the full set (design principles, semantic types, class-per-module, fail fast, never swallow errors, no shims, DI, names, separation of concerns, cohesion, anti-fragile, two-tier messages, docs rules).
