...
- **Audit the project's API-cache layer against the precise cache rule.** The rule is NOT "never cache non-OK responses" — it splits by determinism:
  - **Never cache transient errors (429, 5xx, network)**. A cached outage body is returned as plain data on every later call, retries never fire again, and the failure is permanent even on forced re-runs (observed: 2368 poisoned entries purged).
  - **DO cache deterministic no-route responses (4xx — 404 "cannot route this station", and 3xx disambiguation bodies)**. Re-hitting the endpoint for the same impossible request wastes calls; caching them preserves offline-failure behavior (a resume re-processes failed stations as cache hits, not fresh API calls).
  - Cache 2xx/3xx/4xx minus 429/5xx; on the cache-hit path, evict only legacy TRANSIENT entries (`_cached_status`/`httpStatusCode` in 429 or ≥500), serve everything else.
...
- **The reviewer's enforcement is scoped to `repo_context_files`.** It can only enforce rules present in the files it reads — so the scaffold copies the **canonical global standard** (`docs/coding-standards.md` in the omp-config repo, where this skill lives) **in full** into each repo's `docs/coding-standards.md`, then project rules are added on top. Once the full copy is in place, every rule is enforced (one repo shipped a condensed copy and the bot never commented on style until the full rules were present).
...
- `docs/` standards triplet, checked by PR-Agent:
  - `docs/coding-standards.md` — copy the **canonical global standard** (`docs/coding-standards.md` in the omp-config repo, where this skill lives) **in full**, then add project rules; the reviewer enforces exactly the rules present in the file, so the copy must carry the full set (design principles, semantic types, class-per-module, fail fast, never swallow errors, no shims, DI, names, separation of concerns, cohesion, anti-fragile, two-tier messages, docs rules).
