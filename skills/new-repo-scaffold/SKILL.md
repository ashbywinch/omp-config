...
- **The reviewer enforces only what it reads.** `repo_context_files` is the whole world to the bot — a rule absent from the materialised docs cannot be enforced. The scaffold materialises the **canonical global standard** (`docs/coding-standards.md` in the omp-config repo, where this skill lives) into each repo's `docs/coding-standards.md`, then project-specific rules are added on top. One repo shipped a condensed version and the bot never commented on style until the full rules were materialised.
...
- `docs/` standards triplet, checked by PR-Agent:
  - `docs/coding-standards.md` — materialised from the **canonical global standard** (`docs/coding-standards.md` in the omp-config repo, where this skill lives), then project-specific rules added; the reviewer enforces only what's materialised (design principles, semantic types, class-per-module, fail fast, never swallow errors, no shims, DI, names, separation of concerns, cohesion, anti-fragile, two-tier messages, docs rules).
