...
- **Group into themed sub-epics** when the epic mixes unrelated concerns. Name sub-epics after a phase with a concrete deliverable (`Epic X.Y: <Title>`), parent them at creation (see `skill://epic-quality-standard`), and move insights by read-modify-write on both `Insights` arrays (destination: existing + new; source: cleared).
- **Flag anything unimplementable** — an insight you could not implement correctly from its text alone. Ask the user one ticket at a time; write the clarification into the ticket's `Content`. Never guess.
