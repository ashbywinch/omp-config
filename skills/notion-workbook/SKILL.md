---
name: notion-workbook
description: |
  Core Notion operations via the `ntn` CLI — auth, query patterns, page CRUD,
  property types, error recovery. Reference this from domain-specific skills.
---

# Notion Workbook

Core patterns for Notion operations via `ntn`. Domain skills (epic-creation, task-creation, etc.) reference this for API mechanics rather than inlining them.
