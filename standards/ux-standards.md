...
could not. (2026-08-16: three production bugs — an unscrollable pane, an
initial view showing blank paper, an upstream library's drag collapse — all
passed every static check and were only found by exercising the
interactions.)

**P15 — Success is what the user could perceive, not what the system did.**
...
unreadable, the task is impossible and that is the finding. (2026-08-16: a
walk "completed" a transcription review — decisions confirmed, records
updated — while the letter was a microscopic unreadable strip. Operational
success, perceptual failure.)

**P16 — The reviewer interacts with a site only as the persona could, and
...
"fixed" transcription errors — deleting the model's own strike markers and
guessing a punctuation style — while the document was not visible, and
narrated a reading it never performed.)

**P17 — Review evidence is the rendered pixels, and findings cite the
...
see X" is a finding, never a skip. (2026-08-16: the review surface's
handwriting was verifiable only via canvas pixel-reads and a composed crop
of the source image — the capture environment could not composite it; the
findings that relied on those channels named them.)

**P18 — No dislocating moves.** When the interface moves content in
